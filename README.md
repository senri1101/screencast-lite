...
This project is built for non-engineering users: run one setup script, then use either LaunchAgent mode or Folder Action mode for auto-compression.

## What it does

- Watches these folders via `launchd` (`LaunchAgent` with `WatchPaths` plus a 20-second fallback interval):
  - `~/Desktop`
  - `~/Pictures/screenshots`
- Installs a Folder Action compatibility script (`screen_recording_auto_compress.scpt`) for macOS setups where LaunchAgent cannot read watched folders.
...
6. If LaunchAgent mode does not process recordings, attach `screen_recording_auto_compress.scpt` to the target folder in Folder Actions Setup.
...
2. Save the file to `~/Desktop` or `~/Pictures/screenshots`.
...
- Confirm the file was saved to `~/Desktop` or `~/Pictures/screenshots`.
...
- If scanner logs keep showing `mov=0` while `.mov` files exist in a watched folder, switch to Folder Action mode:
  - Open Folder Actions Setup.
  - Enable Folder Actions.
  - Add `Desktop` or `Pictures/screenshots` in the left panel.
...
- `launch_agent/scan_desktop_recordings.sh`: watched-folder scan entrypoint
